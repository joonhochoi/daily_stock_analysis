...
function getChannelOptions(language: UiLanguage): Array<{ value: NotificationTestChannel; label: string }> {
  return [
    { value: 'wechat', label: language === 'zh' ? '企业微信' : language === 'ko' ? 'WeCom(기업 WeChat)' : 'WeCom' },
    { value: 'feishu', label: language === 'zh' ? '飞书 Webhook' : 'Feishu Webhook' },
    { value: 'telegram', label: 'Telegram' },
    { value: 'email', label: language === 'zh' ? '邮件' : language === 'ko' ? '이메일' : 'Email' },
    { value: 'pushover', label: 'Pushover' },
    { value: 'ntfy', label: 'ntfy' },
    { value: 'gotify', label: 'Gotify' },
    { value: 'pushplus', label: 'PushPlus' },
    { value: 'serverchan3', label: 'ServerChan3' },
    { value: 'custom', label: language === 'zh' ? '自定义 Webhook' : language === 'ko' ? '사용자 Webhook' : 'Custom Webhook' },
