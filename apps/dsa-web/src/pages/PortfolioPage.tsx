...
import { PORTFOLIO_FORM_TEXT, PORTFOLIO_TEXT } from '../locales/featureText';
...
const BROKER_DISPLAY_NAMES: Record<UiLanguage, Record<string, string>> = {
  zh: { huatai: '华泰', citic: '中信', cmb: '招商' },
  en: { huatai: 'Huatai', citic: 'CITIC', cmb: 'China Merchants' },
  ko: { huatai: '화타이', citic: '중신', cmb: '자오상' },
};

function getBrokerLabel(item: PortfolioImportBrokerItem, language: UiLanguage): string {
  const localizedName = BROKER_DISPLAY_NAMES[language][item.broker] || item.displayName?.trim();
  if (!localizedName) return item.broker;
  return `${item.broker} (${localizedName})`;
}
...
const PortfolioPage: React.FC = () => {
  ...
  const formText = PORTFOLIO_FORM_TEXT[language];
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">{formText.newAccount}</h2>
            {hasAccounts ? (
              <button
                type="button"
              ...
              <span className="text-xs text-secondary">{formText.autoSwitchAccount}</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
              ...
              title={formText.createAccountFailed}
          ...
          {accountCreateSuccess ? (
            <InlineAlert
              variant="success"
              className="mt-2 rounded-lg px-2 py-1 text-xs shadow-none"
              title={formText.createAccountSucceeded}
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            ...
            >
                <option value="cn">{formText.marketCn}</option>
                <option value="hk">{formText.marketHk}</option>
                <option value="us">{formText.marketUs}</option>
                <option value="jp">{formText.marketJp}</option>
                <option value="kr">{formText.marketKr}</option>
            </select>
            <button type="submit" className="btn-secondary text-sm" disabled={accountCreating}>
              {accountCreating ? formText.creatingAccount : formText.createAccount}
            </button>
          </form>
        </Card>
      ) : null}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{formText.tradeEntry}</h3>
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={formText.stockSymbolExample} value={tradeForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={tradeForm.side}
                ...
                <option value="buy">{formText.buy}</option>
                <option value="sell">{formText.sell}</option>
              </select>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={formText.quantity} value={tradeForm.quantity}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, quantity: e.target.value }))} required />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={formText.price} value={tradeForm.price}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, price: e.target.value }))} required />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={formText.fee} value={tradeForm.fee}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, fee: e.target.value }))} />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={formText.tax} value={tradeForm.tax}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, tax: e.target.value }))} />
            </div>
            <p className="text-xs text-secondary">{formText.feeTaxHint}</p>
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{formText.submitTrade}</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{formText.cashEntry}</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={cashForm.direction}
                ...
                <option value="in">{formText.cashIn}</option>
                <option value="out">{formText.cashOut}</option>
              </select>
            </div>
            <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={formText.amount}
              value={cashForm.amount} onChange={(e) => setCashForm((prev) => ({ ...prev, amount: e.target.value }))} required />
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={formText.optionalCurrency(writableAccount?.baseCurrency || formText.accountBaseCurrency)} value={cashForm.currency}
              onChange={(e) => setCashForm((prev) => ({ ...prev, currency: e.target.value }))} />
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{formText.submitCash}</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{formText.corporateEntry}</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={formText.stockSymbol} value={corpForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={corpForm.actionType}
                ...
                <option value="cash_dividend">{formText.cashDividend}</option>
                <option value="split_adjustment">{formText.splitAdjustment}</option>
              </select>
            </div>
            {corpForm.actionType === 'cash_dividend' ? (
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder={formText.dividendPerShare}
              ...
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder={formText.splitRatio}
                value={corpForm.splitRatio}
                onChange={(e) => setCorpForm((prev) => ({ ...prev, splitRatio: e.target.value, cashDividendPerShare: '' }))} required />
            )}
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{formText.submitCorporateAction}</button>
          </form>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{formText.brokerCsvImport}</h3>
          <div className="space-y-2">
            {brokerLoadWarning ? (
              <InlineAlert
                variant="warning"
            ...
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={selectedBroker} onChange={(e) => setSelectedBroker(e.target.value)}>
                {brokers.length > 0 ? (
                  brokers.map((item) => <option key={item.broker} value={item.broker}>{getBrokerLabel(item, language)}</option>)
                ) : (
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">{formText.dryRun}</label>
            </div>
            <div className="flex gap-2">
              <button type="button" className="btn-secondary flex-1" disabled={!csvFile || csvParsing} onClick={() => void handleParseCsv()}>
                {csvParsing ? formText.parsing : formText.parseFile}
              </button>
              <button type="button" className="btn-secondary flex-1"
                disabled={!csvFile || !writableAccountId || csvCommitting} onClick={() => void handleCommitCsv()}>
                {csvCommitting ? formText.committing : formText.submitImport}
              </button>
            </div>
            {csvParseResult ? (
              <InlineAlert
                variant={getCsvParseVariant(csvParseResult)}
                title={formText.csvParseResult}
                message={language === 'ko'
                  ? `유효 ${csvParseResult.recordCount}건, 건너뜀 ${csvParseResult.skippedCount}건, 오류 ${csvParseResult.errorCount}건입니다.`
                  : language === 'en'
                    ? `${csvParseResult.recordCount} valid, ${csvParseResult.skippedCount} skipped, ${csvParseResult.errorCount} errors.`
                    : `有效 ${csvParseResult.recordCount} 条，跳过 ${csvParseResult.skippedCount} 条，错误 ${csvParseResult.errorCount} 条。`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
            {csvCommitResult ? (
              <InlineAlert
                ...
                title={csvDryRun ? formText.csvDryRunResult : formText.csvCommitResult}
                message={language === 'ko'
                  ? `${csvDryRun ? '미리 보기 검사' : '실제 저장'}: 저장 ${csvCommitResult.insertedCount}건, 중복 ${csvCommitResult.duplicateCount}건, 실패 ${csvCommitResult.failedCount}건입니다.`
                  : language === 'en'
                    ? `${csvDryRun ? 'Dry-run check' : 'Written'}: ${csvCommitResult.insertedCount} inserted, ${csvCommitResult.duplicateCount} duplicates, ${csvCommitResult.failedCount} failed.`
                    : `${csvDryRun ? '预演检查' : '实际写入'}：写入 ${csvCommitResult.insertedCount} 条，重复 ${csvCommitResult.duplicateCount} 条，失败 ${csvCommitResult.failedCount} 条。`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
          </div>
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{formText.eventLedger}</h3>
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={eventType} onChange={(e) => setEventType(e.target.value as EventType)}>
                <option value="trade">{formText.tradeLedger}</option>
                <option value="cash">{formText.cashLedger}</option>
                <option value="corporate">{formText.corporateLedger}</option>
              </select>
              <button type="button" className="btn-secondary text-sm" onClick={() => void loadEvents()} disabled={eventLoading}>
                {eventLoading ? formText.loading : formText.refreshLedger}
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder={formText.filterBySymbol} value={eventSymbol}
            ...
            {eventType === 'trade' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventSide} onChange={(e) => setEventSide(e.target.value as '' | PortfolioSide)}>
                <option value="">{formText.allTradeSides}</option>
                <option value="buy">{formText.buy}</option>
                <option value="sell">{formText.sell}</option>
              </select>
            ) : null}
            {eventType === 'cash' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventDirection}
                ...
                <option value="">{formText.allCashDirections}</option>
                <option value="in">{formText.cashIn}</option>
                <option value="out">{formText.cashOut}</option>
              </select>
            ) : null}
            {eventType === 'corporate' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventActionType}
                ...
                <option value="">{formText.allCorporateActions}</option>
                <option value="cash_dividend">{formText.cashDividend}</option>
                <option value="split_adjustment">{formText.splitAdjustment}</option>
              </select>
            ) : null}
            <div className="text-[11px] text-secondary">
              {writeBlocked ? formText.writeBlocked : formText.correctionHint}
            </div>
            <div className="max-h-64 overflow-auto rounded-lg border border-white/10 p-2">
              {eventType === 'trade' && tradeEvents.map((item) => (
                <div key={`t-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {item.tradeDate} {item.side === 'buy' ? formText.buy : formText.sell} {item.symbol} · {text.quantity}={item.quantity} · {text.lastPrice}={item.price}
                  </div>
                  {!writeBlocked ? (
                    <button
                      type="button"
                      ...
                      onClick={() => openDeleteDialog({
                        ...
                        id: item.id,
                        message: language === 'ko'
                          ? `${item.tradeDate} ${item.side === 'buy' ? formText.buy : formText.sell} 원장 ${item.symbol}(수량 ${item.quantity}, 가격 ${item.price})을 삭제할까요?`
                          : language === 'en'
                            ? `Delete the ${item.tradeDate} ${item.side} entry for ${item.symbol} (quantity ${item.quantity}, price ${item.price})?`
                            : `确认删除 ${item.tradeDate} 的${formatSideLabel(item.side)}流水 ${item.symbol}（数量 ${item.quantity}，价格 ${item.price}）吗？`,
                      })}
                    >
                      {formText.confirmDelete}
                    </button>
                  ) : null}
                </div>
              ))}
              {eventType === 'cash' && cashEvents.map((item) => (
                <div key={`c-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {item.eventDate} {item.direction === 'in' ? formText.cashIn : formText.cashOut} {item.amount} {item.currency}
                  </div>
                  {!writeBlocked ? (
                    <button
                      type="button"
                      ...
                      onClick={() => openDeleteDialog({
                        ...
                        id: item.id,
                        message: language === 'ko'
                          ? `${item.eventDate} 현금 원장(${item.direction === 'in' ? formText.cashIn : formText.cashOut} ${item.amount} ${item.currency})을 삭제할까요?`
                          : language === 'en'
                            ? `Delete the ${item.eventDate} cash entry (${item.direction} ${item.amount} ${item.currency})?`
                            : `确认删除 ${item.eventDate} 的资金流水（${formatCashDirectionLabel(item.direction)} ${item.amount} ${item.currency}）吗？`,
                      })}
                    >
                      {formText.confirmDelete}
                    </button>
              ...
              {eventType === 'corporate' && corporateEvents.map((item) => (
                <div key={`ca-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {item.effectiveDate} {item.actionType === 'cash_dividend' ? formText.cashDividend : formText.splitAdjustment} {item.symbol}
                  </div>
                  {!writeBlocked ? (
                    <button
                      type="button"
                      ...
                      onClick={() => openDeleteDialog({
                        ...
                        id: item.id,
                        message: language === 'ko'
                          ? `${item.effectiveDate} 기업 활동 ${item.actionType === 'cash_dividend' ? formText.cashDividend : formText.splitAdjustment}(${item.symbol})을 삭제할까요?`
                          : language === 'en'
                            ? `Delete the ${item.effectiveDate} corporate action ${item.actionType} (${item.symbol})?`
                            : `确认删除 ${item.effectiveDate} 的公司行为 ${formatCorporateActionLabel(item.actionType)}（${item.symbol}）吗？`,
                      })}
                    >
                      {formText.confirmDelete}
                    </button>
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title={formText.noLedger}
                      description={formText.noLedgerDescription}
      ...
      <ConfirmDialog
        ...
        title={formText.deleteInvalidLedger}
        message={pendingDelete?.message || formText.confirmDeleteLedger}
        confirmText={deleteLoading ? formText.deleting : formText.confirmDelete}
        cancelText={formText.cancel}
        isDanger
