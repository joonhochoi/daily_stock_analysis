...
const QUICK_QUESTIONS: Record<UiLanguage, Array<{ label: string; skill: string }>> = {
  zh: [
    { label: '用缠论分析茅台', skill: 'chan_theory' }, { label: '波浪理论看宁德时代', skill: 'wave_theory' }, { label: '分析比亚迪趋势', skill: 'bull_trend' }, { label: '箱体震荡技能看中芯国际', skill: 'box_oscillation' }, { label: '分析腾讯 hk00700', skill: 'bull_trend' }, { label: '用情绪周期分析东方财富', skill: 'emotion_cycle' },
  ],
  en: [
    { label: 'Analyze Moutai with Chan theory', skill: 'chan_theory' }, { label: 'Review CATL with wave theory', skill: 'wave_theory' }, { label: 'Analyze BYD trend', skill: 'bull_trend' }, { label: 'Review SMIC with box oscillation', skill: 'box_oscillation' }, { label: 'Analyze Tencent hk00700', skill: 'bull_trend' }, { label: 'Analyze East Money with emotion cycle', skill: 'emotion_cycle' },
  ],
  ko: [
    { label: '찬론으로 마오타이 분석', skill: 'chan_theory' }, { label: '파동 이론으로 CATL 분석', skill: 'wave_theory' }, { label: 'BYD 추세 분석', skill: 'bull_trend' }, { label: '박스권 진동 기법으로 SMIC 분석', skill: 'box_oscillation' }, { label: '텐센트 hk00700 분석', skill: 'bull_trend' }, { label: '감정 주기로 동방재부 분석', skill: 'emotion_cycle' },
  ],
};
...
const joinSkillNames = (names: string[], language: UiLanguage): string => names.join(language === 'zh' ? '、' : ', ');

const getMessageSkillLabel = (msg: Message, language: UiLanguage): string =>
  joinSkillNames(getMessageSkillNames(msg), language);
...
const ChatPage: React.FC = () => {
    ...
    async (stockCode: string) => {
      ...
      try {
        ...
        if (existingStockCode) {
          ...
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(`${t('chat.removeWatchlist')}: ${stockCode}`);
          }
        } else {
          const codes = await systemConfigApi.addToWatchlist(stockCode);
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(`${t('chat.addWatchlist')}: ${stockCode}`);
          }
        }
      } catch {
        if (isMountedRef.current) {
          setWatchlistMessage(t('common.retry'));
        }
      } finally {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || t('chat.compressionLoadError'));
  ...
  const updateContextCompressionEnabled = useCallback(
    async (nextEnabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || t('chat.compressionUpdateError'));
      } finally {
  ...
  const handleSend = useCallback(
    async (overrideMessage?: string, overrideSkillIds?: string[]) => {
      ...
      await startStream(payload, {
        ...
        skillName: joinSkillNames(usedSkillNames, language),
      });
    },
  ...
  const handleQuickQuestion = (q: (typeof QUICK_QUESTIONS)[UiLanguage][number]) => {
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    ...
    if (last.type === 'tool_done')
      return t('chat.toolCompleted', { tool: last.display_name || last.tool || 'tool' });
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          text = step.message || t('chat.thinkingStep', { step: step.step ?? 1 });
            ...
            {sessions.map((s) => (
              <div key={s.session_id} className="session-item-row">
                <button
                  ...
                  aria-label={t('chat.switchConversation').replace('{title}', s.title)}
                ...
                >
                  ...
                  <div className="content">
                    ...
                    <div className="mt-0.5 flex items-center gap-2">
                      <span className="meta">
                        {t('chat.messageCount').replace('{count}', String(s.message_count))}
                      </span>
                      {s.last_active && (
                        <>
                ...
                <button
                  ...
                  aria-label={t('chat.deleteConversation').replace('{title}', s.title)}
                >
                  <svg
  ...
  const selectedSkillSummary = selectedSkillIds.length > 0
    ? joinSkillNames(getSkillNames(selectedSkillIds), language)
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                ...
                <Tooltip content={t('chat.sendNotification')}>
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: t('chat.notificationSent') }, 3000);
                        } catch (err) {
                      ...
                      aria-label={t('chat.sendNotification')}
                    >
                      {sending ? (
                        <svg
                ...
                >
                  ...
                  >
                    {msg.role === 'assistant' && skillLabel && (
                      <div className="mb-2">
                        <Badge variant="info" className="chat-skill-badge shadow-none" aria-label={t('chat.skill').replace('{name}', skillLabel)}>
                          <svg
                            className="w-3 h-3"
                            fill="none"
                            stroke="currentColor"
          ...
          {showJumpToBottom && (
            <div className="pointer-events-none absolute bottom-[5.75rem] right-4 z-20 md:bottom-24 md:right-6">
              <button
                ...
                aria-label={t('chat.latestMessage')}
              >
                <svg
            ...
            <div className="space-y-3">
              ...
              {isFollowUpContextLoading ? (
                <InlineAlert
                  ...
                  message={t('chat.followupMessage')}
              ...
              {contextCompressionError ? (
                <InlineAlert
                  variant="danger"
                  title={t('chat.compressionSaveError')}
