...
> UI language resolution is: explicit localStorage value (`zh`, `en`, or `ko`) -> browser language (`navigator.languages` / `navigator.language`) -> default `zh`.
...
- **UI Language Switch** - Cycle UI language (`zh`/`en`/`ko`) on login page, shell/navigation, settings page, and shared controls; this switch is independent of `REPORT_LANGUAGE`.
