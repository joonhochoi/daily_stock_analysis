...
| `REPORT_LANGUAGE` | 报告输出语言：`zh`(默认中文) / `en`(英文) / `ko`(韩语)；会同步影响 Prompt、模板、通知 fallback 与 Web 报告页固定文案。仓库自带 `00-daily-analysis.yml` 已显式映射该变量，直接在 Actions Secrets/Variables 中配置即可生效 | 可选 |
...
> 1) 明确选择（`localStorage.dsa.uiLanguage`，支持 `zh`/`en`/`ko`）
> 2) 浏览器语言检测（`navigator.languages` / `navigator.language`，`zh-*`、`en-*` 或 `ko-*`）
> 3) 默认回退 `zh`。
...
- 📝 **配置管理** - 查看/修改自选股列表
- 🧭 **界面语言切换** - 登录态与退出态均支持界面语言快速切换（`zh` / `en` / `ko`），独立于 `REPORT_LANGUAGE`，用于静态 UI 文案与导航骨架
...
- Web 语言状态采用两层机制：`dsa.uiLanguage`（浏览器持久化）与 `REPORT_LANGUAGE`（报告输出）解耦。
  - `dsa.uiLanguage` 只决定 WebUI 文案与导航语言（`zh` / `en` / `ko`），取值优先级为本地持久化值 -> 浏览器语言 -> 默认 `zh`。
